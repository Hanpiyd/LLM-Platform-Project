...
class OpenAIModel:
    ...
    def load(self) -> None:
        ...
        try:
            ...
            self.loaded = True
            logger.info(f"OpenAI model {self.model_id} loaded successfully")
        except ImportError:
    ...
    def unload(self) -> None:
        ...
        self.client = None
        self.loaded = False
        logger.info(f"OpenAI model {self.model_id} unloaded")
        
    def generate(self, system_prompt: str, user_question: str, params: dict = None) -> str:
        if not self.loaded:
            raise RuntimeError(f"OpenAI model {self.model_id} is not loaded")
        try:
            messages = []
            if system_prompt:
